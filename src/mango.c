...
mapnotify(struct wl_listener *listener, void *data) {
	...
	wlr_log(WLR_ERROR, "[canvas_debug] mapnotify start: client=%p mon=%p", c, c->mon);
	...
	wlr_log(WLR_ERROR, "[canvas_debug] mapnotify: scene tree created client=%p", c);

	client_get_geometry(c, &c->geom);
	wlr_log(WLR_ERROR, "[canvas_debug] mapnotify: got geometry %dx%d client=%p", c->geom.width, c->geom.height, c);

	if (client_is_x11(c))
		init_client_properties(c);

	if (c->mon && c->mon->canvas_in_overview) {
		wlr_log(WLR_ERROR, "[canvas_debug] mapnotify: canvas in overview, toggling off client=%p", c);
	...
	wlr_log(WLR_ERROR, "[canvas_debug] mapnotify: before iter_xdg_scene_buffers client=%p", c);
	wlr_scene_node_for_each_buffer(&c->scene_surface->node,
								   iter_xdg_scene_buffers, c);
	wlr_log(WLR_ERROR, "[canvas_debug] mapnotify: after iter_xdg_scene_buffers client=%p", c);
